    ...
    <section>
      <header className='my-11 flex'>
        <div>
          ...
          <p className='text-2xl'>{phonetics[0].text}</p>
        </div>
      </header>
      <div>
        {meanings.map((meaning, index) => (
          <div key={index}>
            <header className='flex flex-wrap items-center my-5'>
              <h3 className="text-2xl font-bold leading-none italic mr-5">{ meaning.partOfSpeech }</h3>
              <hr className='grow' />
            </header>
            <section className='py-5'>
              <h4 className='text-xl mb-5'>Meaning</h4>
              ...
              <section className='flex'>
                ...
                <div>
                  {meaning.synonyms.map((synonym, index) => (
                    <span key={index} className='text-lg mr-2 font-bold hover:cursor-pointer hover:underline'>{ synonym }</span>
                  ))}
                </div>
              </section>
            )}
              ...
              <section className='flex'>
                ...
                <div>
                  {meaning.antonyms.map((antonym, index) => (
                    <span key={index} className='text-lg mr-2 font-bold hover:cursor-pointer hover:underline'>{ antonym }</span>
                  ))}
                </div>
              </section>
            )}
              ...
              {Array.from(sources).map((url, index) => (
                <li key={index} className='text-neutral-800'>
                  <a href={url} target="_blank" rel="noopener noreferrer" className='flex items-center group'>
                    <span className='mr-1 group-hover:underline'>{ url }</span>
